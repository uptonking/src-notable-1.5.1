...
class Tagbox extends React.PureComponent<
  { onClick: Function; icon: string; title: string; placeholder: string },
  {}
> {
  $wrapper;
  $tagbox;
  ...
  render() {
    ...
    return (
      <div className="multiple joined fluid">
        <div
          className="tagbox bordered fluid"
          data-options='{"addOnBlur": true}'
        >
          <input name="name" defaultValue="" className="hidden" />
          <div className="tagbox-tags">
            <input
              placeholder={placeholder}
              className="tagbox-partial autogrow compact small"
            />
          </div>
        </div>
