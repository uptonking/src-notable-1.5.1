...
interface TreeProps {
  /** children值应该是react.component类型，用在render方法中 */
  children:any;
  data: any[];
  className?: string;
  FallbackEmptyComponent?;
...
/**
 * 基于react-window实现的树型组件，用来操作多级tag
 */
class Tree extends React.Component<TreeProps, TreeState> {
  ...
  shouldComponentUpdate(propsNext, stateNext) {
    return (
      this.state.height !== stateNext.height ||
      !isShallowEqual(this.state.items, stateNext.items)
    );
  }
  ...
  scrollToItem = (event: Event, index: number) => {
    ...
    if (index === 0) {
      //FIXME: https://github.com/bvaughn/react-window/issues/136
      setTimeout(() => {
  ...
  getItemKey = (index: number) => {
    ...
    if (this.props.getItemKey) {
      return isNumber
        ? this.props.getItemKey(this.getItem(index)) || index
        : this.props.getItemKey(index) || this.getItemIndex(index);
    } else {
  ...
  getItems = (data: any[], level: number = 0) => {
    if (this.props.isFlat)
      return {
        items: data.filter(this.filterItem as any),
        levels: [],
        isLeafs: [],
      }; //TSC
    ...
    data.forEach((item) => {
      ...
      children.forEach((item) => {
        const {
          items: childrenItems,
          levels: childrenLevels,
          isLeafs: childrenIsLeafs,
        } = this.getItems([item], level + 1);
  ...
  areItemsKeysEqual(x, y) {
    ...
    return x.every(
      (item, index) => this.getItemKey(item) === this.getItemKey(y[index])
    );
  }
  ...
  updateItems = (props, callback?) => {
    ...
    if (
      this.props.isKeyed &&
      this.areItemsKeysEqual(items, this.state.items) &&
      (this.props.isFlat ||
        (_.isEqual(levels, this.state.levels) &&
          _.isEqual(isLeafs, this.state.isLeafs)))
    )
      return; // Skipping unnecessary update, the keys didn't change
