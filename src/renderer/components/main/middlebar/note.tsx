...
const Note = ({
  ...
  const onClick = (event) =>
    Svelto.Keyboard.keystroke.hasCtrlOrCmd(event)
      ? toggleNote(note)
      : event.shiftKey
  ...
  return (
    <div
      style={style}
      className={`note ${
        !isMultiEditorEditing && isActive ? "label" : "button"
      } ${
        (isMultiEditorEditing ? isSelected : isActive) ? "active" : ""
      } list-item`}
      data-checksum={note.checksum}
      data-filepath={note.filePath}
