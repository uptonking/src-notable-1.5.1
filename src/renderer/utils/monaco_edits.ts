...
const Edits = {
  ...
  // We are diffing `before` and `after`, instead of replacing `before` with `after` naively, in order to avoid having the cursors move unnecessarily
  makeReplace(before: string, after: string, lineNr: number = 0): monaco.editor.IIdentifiedSingleEditOperation[] {
