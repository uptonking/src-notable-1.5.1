...
/**
 * Monaco编辑器的配置选项, 主要包括编辑,快捷键,主题等方面
 */
const Monaco = {
  editorOptions: {
  ...
  keybindings: {
    ...
    "editor.toggleMinimap": {
      ...
      handler(accessor, editor: MonacoEditor) {
        ...
        Monaco.editorOptions.minimap.enabled = !Monaco.editorOptions.minimap
          .enabled;

        Settings.set(
          "monaco.editorOptions.minimap.enabled",
          Monaco.editorOptions.minimap.enabled
        );
  ...
  keybindingsPatched: {
    ...
    "editor.action.moveLinesDownAction": (cmd) => {
      cmd._kbOpts.primary = cmd._kbOpts.linux.primary =
        monaco.KeyMod.WinCtrl | monaco.KeyMod.Alt | monaco.KeyCode.DownArrow;
      cmd._kbOpts.mac = {};
      cmd._kbOpts.mac.primary =
    ...
    "editor.action.moveLinesUpAction": (cmd) => {
      cmd._kbOpts.primary = cmd._kbOpts.linux.primary =
        monaco.KeyMod.WinCtrl | monaco.KeyMod.Alt | monaco.KeyCode.UpArrow;
      cmd._kbOpts.mac = {};
      cmd._kbOpts.mac.primary =
        monaco.KeyMod.CtrlCmd | monaco.KeyMod.WinCtrl | monaco.KeyCode.UpArrow;
    },
  ...
  initTokenizers() {
    ...
    LanguageMarkdown.language.tokenizer.root.unshift([
      /^(\s{0,3})(#+)((?:[^\\#]|@escapes)+)((?:#+)?)/,
      ["white", "keyword.title", "title", "keyword.title"],
    ]);
  },
