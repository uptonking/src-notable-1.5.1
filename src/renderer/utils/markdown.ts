...
/**
 * markdown操作工具对象，主要包括extensions、converters、判断检查等工具
 */
const Markdown = {
  re: /_.*?_|\*.*?\*|~.*?~|`.*?`|<.*?>|:\w+?:|^\s*>|^\s*#|\[.*?\]|--|==|```|~~~|^\s*\d+\.|^\s*[*+-]\s|\n\s*\n/m,
  wrapperRe: /^<p>(.*?)<\/p>$/,

  /** 扩展markdown操作的功能, 包括todo、高亮、tex、流程图 */
  extensions: {
    utilities: {
      ...
      isInside(re: RegExp, str: string, index: number) {
        // Checks if the index is inside the ranges matched by the regex in the string
      ...
      isInsideAnchor(str: string, index: number) {
        return Markdown.extensions.utilities.isInside(
          Markdown.extensions.utilities.anchorOutputRe,
          str,
          index
        );
      },
      ...
      isInsideCode(str: string, index: number, language: boolean = false) {
        const re = language
          ? Markdown.extensions.utilities.codeLanguageRe
          : Markdown.extensions.utilities.codeOutputRe;
      ...
      toggleCheckbox(str: string, nth: number, force?: boolean) {
        ...
        while ((checkbox = checkboxLanguageRe.exec(str))) {
          if (
            Markdown.extensions.utilities.isInsideCode(
              str,
              checkbox.index,
              true
            )
          ...
          force = _.isBoolean(force)
            ? force
            : !checkboxCheckedRe.test(checkbox[0]);

          const checkboxNext = checkbox[0].replace(
            checkboxCheckmarkRe,
            force ? "[x]" : "[ ]"
          );

          return `${str.slice(0, checkbox.index)}${checkboxNext}${str.slice(
            checkbox.index + checkbox[0].length,
            Infinity
          )}`;
        }
      ...
      return [
        {
          ...
          replace(match, $1, $2, $3, $4, $5, index, content) {
            ...
            if (
              Markdown.extensions.utilities.isInsideCode(content, index, false)
            )
              return match;
            if (Markdown.extensions.utilities.isInsideAnchor(content, index))
              return match; // In order to better support encoded emails
      ...
      return [
        {
          ...
          replace(match, $1, $2, index, content) {
            if (
              Markdown.extensions.utilities.isInsideCode(content, index, true)
            )
              return match;
            const filePath = path.resolve(notesPath, $2);
            if (filePath.startsWith(attachmentsPath)) {
              return `[${$1}](${attachmentsToken}/${filePath.slice(
                attachmentsPath.length
              )})`;
            } else if (filePath.startsWith(notesPath)) {
              return `[${$1}](${notesToken}/${filePath.slice(
                notesPath.length
              )})`;
            } else {
        ...
        },
        {
          ...
          replace(match, $1, $2, $3, $4, $5) {
            ...
            if (filePath.startsWith(attachmentsPath)) {
              return `<${$1} ${$2} ${$3}="${attachmentsToken}/${filePath.slice(
                attachmentsPath.length
              )}" ${$5}>`;
            } else if (filePath.startsWith(notesPath)) {
              return `<${$1} ${$2} ${$3}="${notesToken}/${filePath.slice(
                notesPath.length
              )}"${$5}>`;
            } else {
              return `<${$1} ${$2} ${$3}="file://${encodeFilePath(
                filePath
              )}"${$5}>`;
            }
          },
    ...
    encodeSpecialLinks() {
      // Or they won't be parsed as images/links whatever
      ...
      return [
        {
          ...
          replace(match, $1, $2, $3, $4) {
            ...
            return `<${$1}${$2}src="file://${encodeFilePath(
              filePath
            )}" class="attachment" data-filename="${$3}"${$4}>`;
          },
        },
        {
          // Link Button
          ...
          replace(match, $1, $2, $3) {
            ...
            return `<a${$1}href="file://${encodeFilePath(
              filePath
            )}" class="attachment button highlight" data-filename="${$2}"${$3}><i class="icon small">paperclip</i><span>${basename}</span></a>`;
          },
        },
        {
          // Link
          ...
          replace(match, $1, $2, $3) {
            ...
            return `<a${$1}href="file://${encodeFilePath(
              filePath
            )}" class="attachment" data-filename="${$2}"${$3}><i class="icon xsmall">paperclip</i>`;
          },
        },
      ];
    },
      ...
      return [
        {
          ...
          replace(match, $1, $2, $3) {
            ...
            return `<a${$1}href="file://${encodeFilePath(
              filePath
            )}" class="note button highlight" data-filepath="${filePath}"${$3}><i class="icon small">note</i><span>${basename}</span></a>`;
          },
        },
        {
          // Link
          ...
          replace(match, $1, $2, $3) {
            ...
            return `<a${$1}href="file://${encodeFilePath(
              filePath
            )}" class="note" data-filepath="${filePath}"${$3}><i class="icon xsmall">note</i>`;
          },
        },
      ];
    },
      ...
      return [
        {
          ...
          regex: /<a(.*?)href="(.*?)>/g,
          replace(match, $1, $2) {
            if ($2.startsWith("#") || isAbsoluteUrl($2)) {
              // URL fragment or absolute URL
              return match;
            } else {
  ...
  /**
   * markdown转换成html，单向的，
   * showndown的html2md的api没有文档说明，要使用master分支的converter.makeMarkdown(htmlText)
   */
  converters: {
    preview: _.memoize(() => {
      const {
  ...
  is: (str: string): boolean => {
    // Checks if `str` _could_ be using some Markdown features, it doesn't tell reliably when it actually is, only when it isn't. Useful for skipping unnecessary renderings
  ...
  render: (str: string, limit: number): string => {
    ...
    return Markdown.converters
      .preview()
      .makeHtml(Markdown.limiter(str, limit).trim());
  },
  ...
  strip: (str: string, limit: number): string => {
    ...
    return Markdown.converters
      .strip()
      .makeHtml(Markdown.limiter(str, limit))
      .trim()
      .replace(Markdown.wrapperRe, "$1");
  },
