...
class Note extends Container<NoteState, MainCTX> {
  ...
  _inferTitleFromFilePath(filePath: string): string {
    return path
      ...
      .replace(/\∕/g, "/")
      .replace(/\s{2,}/g, " "); // Substituting back dash-like characters
  }
  ...
  _inferTitleFromLine(
    ...
    return line
      ? entities.decode(Markdown.strip(line.trim()).replace(/\s{2,}/g, " ")) ||
          fallback
      : fallback;
  }
  ...
  new = async (title: string = "Untitled") => {
    ...
    const { filePath, fileName } = await Path.getAllowedPath(
        notesPath,
        `${title}${ext}`
      ),
      ...
      name =
        searchQuery && !searchNotes.length
          ? searchQuery
    ...
    if (
      ...
      // Trying to put the new note in the current tag
    ...
    if (noteAdded) {
      // It's possible that it was never found
  ...
  duplicate = async (
    note: NoteObj | undefined = this.state.note,
    resetTemplate: boolean = false
  ) => {
    if (!note) return;
    ...
    if (resetTemplate) {
      ...
      duplicateNote.metadata.tags = _.without(
        duplicateNote.metadata.tags,
        ...tagsTemplates
      );
    }
  ...
  delete = async (
    ...
    const title = this.getTitle(note),
      attachments = this.getAttachments(note)
        .map((attachment) => this.ctx.attachment.get(attachment))
        .filter(_.identity) as AttachmentObj[],
      attachmentsUnique = attachments.filter(
        (attachment) => this.ctx.attachment.getNotes(attachment).length < 2
      );
    ...
    if (
      ...
      !Dialog.confirm(
        `Are you sure you want to permanently delete "${title}"${
          attachmentsUnique.length
            ? ` and ${
                attachmentsUnique.length === attachments.length
                  ? "all"
                  : `${attachmentsUnique.length} out of ${attachments.length}`
              } attachments`
            : ""
        }?`
      )
    ...
    await Promise.all(
      attachmentsUnique.map((attachment) =>
        this.ctx.attachment.delete(attachment, true)
      )
    );
  ...
  /** 从notes对象中获取指定路径的note数据 */
  get = (filePath?: string): NoteObj | undefined => {
    return filePath ? this.ctx.notes.get()[filePath] : this.state.note;
  };

  getWait = (filePath?: string): Promise<NoteObj | undefined> => {
    // Get the note waiting for it to become available (maybe it's being written to disk or something)
  ...
  is = (note1?: NoteObj, note2?: NoteObj, loose: boolean = false): boolean => {
    return (
      ...
      (!!note1 &&
        ...
        (loose || note1.content === note2.content))
    );
  };
  ...
  getTags = (
    ...
    return startingWith
      ? tags.filter(
          (tag) =>
            tag === startingWith ||
            tag.startsWith(`${startingWith}${Tags.SEPARATOR}`)
        )
      : tags;
  };
  ...
  addAttachments = async (
    ...
    for (let srcPath of filePaths) {
      ...
      if (!attachment || attachment.filePath !== srcPath) {
        // Not an existing attachment

        attachment = this.ctx.attachment.read(srcPath);

        const { filePath: dstPath, fileName } = await Path.getAllowedPath(
          attachmentsPath,
          attachment.fileName
        );
      ...
      nextNote.metadata.attachments = Attachments.sort(
        nextNote.metadata.attachments
      );
    }
  ...
  _removeAttachment = (
    ...
    // Just remove the attachment from the note //UGLY

    if (!note) return;

    const nextNote = _.cloneDeep(note);

    nextNote.metadata.attachments = _.without(
      nextNote.metadata.attachments,
      attachment.fileName
    );
    ...
    if (
      !Dialog.confirm(
        `Are you sure you want to ${
          willDeleteFile ? "permanently delete" : "remove, just from this note,"
        } "${attachment.fileName}"?`
      )
    )
      return;
  ...
  toggleDeleted = (
    note: NoteObj | undefined = this.state.note,
    force: boolean = !this.isDeleted(note)
  ) => {
    if (!note) return;
  ...
  toggleFavorite = async (
    note: NoteObj | undefined = this.state.note,
    force: boolean = !this.isFavorited(note)
  ) => {
    if (!note) return;
  ...
  togglePin = (
    note: NoteObj | undefined = this.state.note,
    force: boolean = !this.isPinned(note)
  ) => {
    if (!note) return;
  ...
  toggleCheckboxNth = (
    ...
    const plainContent = this.getPlainContent(note),
      plainContentNext = Markdown.extensions.utilities.toggleCheckbox(
        plainContent,
        nth,
        force
      );
  ...
  save = async (
    ...
    const nextNote = _.cloneDeep(note),
      ...
      title =
        titleLinePrev !== titleLineNext
          ? this._inferTitleFromLine(titleLineNext)
          : note.metadata.title,
    ...
    if (didTitleChange) {
      ...
      const ext = path.extname(note.filePath) || Config.notes.ext,
        { filePath } = await Path.getAllowedPath(
          path.dirname(nextNote.filePath),
          `${title}${ext}`
        );
  ...
  /** 将最新的note数据写到本地文件 */
  write = async (note: NoteObj, modified: Date = new Date()) => {
    // Remember to update the export methods when modifying the written metadata

    const metadata: any = _.clone(note.metadata);
   ...
   */
  sanitize = async (
    note: Pick<NoteObj, Exclude<keyof NoteObj, "metadata">> & {
      metadata: Partial<NoteObj["metadata"]>;
    }
  ): Promise<NoteObj> => {
    ...
    if (!note.metadata.created || !_.isDate(note.metadata.created)) {
      note.metadata.created = note.metadata.created
        ? new Date(note.metadata.created)
        : note.metadata.stat
        ? new Date(note.metadata.stat.birthtimeMs)
        : new Date();

      if (_.isNaN(note.metadata.created.getTime()))
        note.metadata.created = new Date();
    }

    if (!note.metadata.modified || !_.isDate(note.metadata.modified)) {
      note.metadata.modified = note.metadata.modified
        ? new Date(note.metadata.modified)
        : note.metadata.stat
        ? new Date(note.metadata.stat.mtimeMs)
        : new Date();

      if (_.isNaN(note.metadata.modified.getTime()))
    ...
    if (!_.isArray(note.metadata.attachments)) {
      ...
      note.metadata.attachments = Attachments.sort(
        this.sanitizeAttachments(note.metadata.attachments)
      );
    }
    ...
    if (!_.isArray(note.metadata.tags)) {
      ...
      note.metadata.tags = Tags.sort(
        this.sanitizeTags(note.metadata.tags)
      ) as string[];
    }
  ...
  sanitizeAttachments = (attachments: string[]): string[] => {
    return attachments.filter(_.isString).filter(_.identity);
  };
  ...
  replace = async (
    ...
    const isSameFile = note.filePath === nextNote.filePath,
      isActiveNote =
        this.state.note && this.state.note.filePath === note.filePath;

    if (!isSameFile) {
      if (await File.exists(note.filePath)) {
        if (!(await File.exists(nextNote.filePath))) {
          // Avoiding unlinking the file in order to prevent data losses
  ...
  update = (prevIndex?: number) => {
    ...
    const index =
        prevIndex && prevIndex >= 0 ? Math.min(notes.length - 1, prevIndex) : 0,
