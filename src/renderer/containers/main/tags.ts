...
class Tags extends Container<TagsState, MainCTX> {
  ...
  _toggleNote = (
    ...
    function toggle(parent: TagsObj, key: string, deletable: boolean = false) {
      ...
      const index = tag.notes.findIndex(
        (n) => n.checksum === note.checksum && n.filePath === note.filePath
      ); //FIXME: This should actually be `tag.notes.indexOf ( note )` but for some reason some times it doesn't work
      if (add) {
        if (index === -1) {
          tag.notes.push(note);
        }
    ...
    if (this.ctx.note.isDeleted(note)) {
      ...
      if (!tagsAll.length) {
        ...
        tagsAll.forEach((tag) => {
          ...
          parts.reduce(
            (tags: TagsObj | undefined, tag: string, index: number) => {
              if (!tags) return;
              ...
              const isSpecial =
                !index && (tag === NOTEBOOKS || tag === TEMPLATES);
  ...
  refresh = () => {
    ...
    tags[FAVORITES] = {
      ...
      collapsed: false,
      path: FAVORITES,
      name: TagSpecialsNames.FAVORITES,
      notes: [],
      tags: {},
    };
    tags[NOTEBOOKS] = {
      icon: "notebook",
      iconCollapsed: "notebook_multiple",
      collapsed: false,
      path: NOTEBOOKS,
      name: TagSpecialsNames.NOTEBOOKS,
      notes: [],
      tags: {},
    };
    tags[TAGS] = {
      collapsed: false,
      path: TAGS,
      name: TagSpecialsNames.TAGS,
      notes: [],
      tags: {},
    };
    tags[TEMPLATES] = {
      icon: "tag_outline",
      iconCollapsed: "tag_outline_multiple",
      collapsed: false,
      path: TEMPLATES,
      name: TagSpecialsNames.TEMPLATES,
      notes: [],
      tags: {},
    };
    tags[UNTAGGED] = {
      ...
      collapsed: false,
      path: UNTAGGED,
      name: TagSpecialsNames.UNTAGGED,
      notes: [],
      tags: {},
    };
    tags[TRASH] = {
    ...
    Object.values(this.ctx.notes.get()).forEach((note) =>
      this._toggleNote(tags, note, true)
    );
