...
class Attachment extends Container<AttachmentState, MainCTX> {
  ...
  getNotes = (attachment: AttachmentObj): NoteObj[] => {
    ...
    return notes.filter((note) =>
      this.ctx.note.hasAttachment(note, attachment)
    );
  };

  openInApp = (attachment: AttachmentObj) => {
    if (!attachment)
      return Dialog.alert("This attachment is no longer stored in disk");
  ...
  delete = async (attachment?: AttachmentObj, justFile: boolean = false) => {
    ...
    if (await File.exists(attachment.filePath))
      File.unlink(attachment.filePath);
    ...
    return Promise.all(
      notes.map((note) => this.ctx.note._removeAttachment(note, attachment))
    );
  };
}
